...
public class DeployerHttpException extends RuntimeException {
	...
	static DeployerHttpException rateLimit(int statusCode, HttpHeaders headers) {
		return new DeployerRateLimitHttpException(statusCode,
			...
			headers.firstValue("X-RateLimit-Reset").map(Long::valueOf).map(Instant::ofEpochSecond).orElse(Instant.MIN));
	}

	static DeployerHttpException rateLimit(int statusCode, Instant reset) {
		return new DeployerRateLimitHttpException(statusCode, -1, -1, reset);
	}
	...
	public static class DeployerRateLimitHttpException extends DeployerHttpException {
		...
		public DeployerRateLimitHttpException(int httpStatusCode, long remaining, long limit, Instant reset) {
